...
Android APK that embeds a Termux-style Linux bootstrap environment, installs Codex on first run, and presents the codexapp UI inside a WebView.
...
If you want to pre-bundle the codexapp server in the APK so users don't need to `npm install` it on first run:
...
3. Run `npm install -g @openai/codex codexapp`
